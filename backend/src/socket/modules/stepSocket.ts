...
export function syncStepStateSelf(socket: Socket, roomId: RoomId, roomStateManager: RoomStateManager) {
...
export function syncStepStateAll(io: Server, roomId: RoomId, roomStateManager: RoomStateManager) {
    const stepIndex = roomStateManager.getStepIndex(roomId);
    io.to(roomId).emit(`${SocketEvent.STEP_STATE_SYNC_ALL}`, stepIndex);
    logger.info(`Sent ${SocketEvent.STEP_STATE_SYNC_ALL} stepIndex: ${stepIndex}`);
}

...
**Value reference breaks reverse edges — never `import` a downstream module's component into an upstream one.** A `<X>HoverCard.vue` (or any component) owned by a *derived/downstream* module (`analysis`) must not be `import`ed by a *source/canonical* module's UI (`match`, `board`) — that makes the source depend on its own consumer (`match → analysis`), inverting the dependency arrow. Instead the source module's `.vue` injects the slot-wrapper via `useCharacterHoverWrapper()` and renders it as `<component :is="CharacterHoverWrapper" :character-key :disabled>`; the **composition view** that assembles both (`banPick`, `playerProfile`) calls `provideCharacterHoverWrapper(CharacterHoverCard)` in its `<script setup>`. The view is allowed to import the derived component directly (composition views orchestrate any domain); the source module stays dependency-clean and degrades to `Passthrough` (renders the slot, no hover) wherever no provider exists.

Checklist when you reach for this: **(1)** every mount site of the consuming component must have an ancestor calling `provide*` — a derived module's component embedded in a modal that two views render (`MatchHistoryModal` in `banPick` *and* `playerProfile`) needs the `provide*` in **both** views, else the second silently degrades to `Passthrough`. **(2)** The injected component's props must match the `Passthrough` stub's props (`characterKey` required, `disabled` optional) so both render paths are interchangeable.

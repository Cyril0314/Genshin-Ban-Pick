// src/modules/analysis/ui/composables/usePlayerStyleExplorer.ts
...
const logger = createLogger('analysis.ui.playerStyleExplorer');
...
export function usePlayerStyleExplorer() {
    ...
    const playerStyle = ref<IPlayerStyle>();
    ...
    watch(selectedScope, async () => {
        ...
        if (selectedScope.value.type === 'Player') {
            playerStyle.value = await analysisUseCase.fetchPlayerStyle(selectedScope.value.player);

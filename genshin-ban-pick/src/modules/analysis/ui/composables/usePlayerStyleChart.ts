...
const logger = createLogger('analysis.ui.playerStyle');

export function usePlayerStyleChart(identity: MaybeRefOrGetter<PlayerIdentity | undefined>) {
    const analysisUseCase = useAnalysisUseCase();
    const { tooltipStyle } = useEchartTheme();
    const designTokens = useDesignTokens();

    const isLoading = ref(false);
    const profile = ref<IPlayerStyle | undefined>(undefined);
    ...
    watch(
        ...
        async (id, _old, onCleanup) => {
            ...
            try {
                const result = await analysisUseCase.fetchPlayerStyle(id);

...
export function useRandomPull() {
  function randomPull(zoneType: ZoneType, roomSetting: IRoomSetting, boardImageMap: Record<number, string>, filteredImageIds: string[]): { imgId: string; zoneId: number } | null {
    ...
    switch (zoneType) {
      case ZoneType.UTILITY:
        zoneId = findNextUtilityZoneId(roomSetting.zoneSchema.utilityZones, boardImageMap)
        break;

      case ZoneType.PICK:
        zoneId = findNextBanPickStepZoneId('pick', roomSetting.banPickSteps, roomSetting.zoneSchema.zoneMetaTable, boardImageMap)
        break;

      case ZoneType.BAN:
        zoneId = findNextBanPickStepZoneId('ban', roomSetting.banPickSteps, roomSetting.zoneSchema.zoneMetaTable, boardImageMap)
        break;
    }
  ...
  function findNextUtilityZoneId(utilityZones: IZone[], boardImageMap: Record<number, string>): number | null {
